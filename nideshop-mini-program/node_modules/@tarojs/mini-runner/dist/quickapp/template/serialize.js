...
const createNodeCode = (name, children, content, attrsCode, $delete, url) => {
    ...
    // 针对不支持子组件的组件，子组件需注释
    childrenNodes = $delete ? utils_1.addComment(childrenNodes, url) : childrenNodes;
    return `<${name}${attrsCode}>${content || ''}${childrenNodes}</${name}>`;
};
const walk = (node) => {
    const attrsCode = createAttrsCode(node.attributes);
    const nodeCode = createNodeCode(node.name, node.children, node.content, attrsCode, node.$delete, node.url);
    return nodeCode;
};
